...
public class BORWindow : MonoBehaviour
{
	...
	public void Upgrade() 
	{
		...
		{
			CastBar.Instance.StartCast("Refining...", 2f, EndUpgrade);
		}
	}
	...
	private void EndUpgrade() 
	{
		if (toUpgrade is Sword) 
		{
			...
			MessageFeedManager.instance.WriteMessage("Refined succesfuly! New quality: " + (ToUpgrade as Sword).EquipmentQuality.ToString(), FontSize.NORMAL, MessageType.Warning, 2f);
		...
		else 
		{
			...
			MessageFeedManager.instance.WriteMessage("Refined succesfuly! New quality: " + (ToUpgrade as Armor).EquipmentQuality.ToString(), FontSize.NORMAL, MessageType.Warning, 2f);
		}
		TakeMats();	
		SetMats();
		book.Remove();
	...
	private bool HaveMats() 
	{
		...
		MessageFeedManager.instance.WriteMessage("You don't have enough materials.", FontSize.NORMAL, MessageType.Warning, 2f);
		return false;
	}
